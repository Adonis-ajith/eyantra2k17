...
void source_to_target(int8_t source,int8_t taarget)
{
	...
	{	
		next_node=path[y];
		send_data(next_node+50);
		
		lcd_print(2,1,current_node,2);   // LCD print current node in 2nd row 1,2 column.
		lcd_print(2,4,next_node,2);      // LCD print next node in 2nd row 1,2 column.
		...
		if(obstacle_flag==1)    //obstacles was detected
		{
			...
			send_data(200+current_node);
			send_data(200+next_node);			
			break;
		}
		
		current_node=next_node;			//Current node is updated
		send_data(current_node);
		if(path[y]!=-1)
		stop();		
	}
...
{
	int l,r,m,th=40;	//th =threshold adc value of line follower sensor. Below th --->white layer. Above th--->black layer
	int max=170,vr=0,vl=0;
	int i=0;
	while(1)
	{
		...
		vr = (int)((r*r*0.001461)+(r*-1.083)+258.9);		//bot2
		vl = (int)((l*l*0.0009841)-(l*1.011)+255.9);		//bot2
	
	
		vl=constrain(vl,250,200);
		vr=constrain(vr,250,200);		
		i++;
			//	lcd_print(2,1,i,3);
