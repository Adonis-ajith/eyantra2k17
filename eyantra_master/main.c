...
#define note_string_len 10			//The length of the given MNP string

/*********************************************************************/

uint8_t note_string[10]={14, 5, 18, 1, 19, 9, 2, 7, 6, 6};

uint8_t note_flag[15];
...
int8_t current_node=0;	

int8_t previous_node=23;

int8_t next_node;										//current_node :To store the node at which the bot currently at

int8_t current_node_slave = 12;

int8_t next_node_slave;
...
int8_t mnp_for_slave=0;

int8_t note_num_slave;
...
{					
	...
	for(n=0;n<15;)
	{
		s = overall_shortest_path(current_node,current_node_slave,note_string[n],note_string[n+1]);
				
		if (s == 'A')
		{
			node_for_note(current_node_slave,note_string[n+1]);
			note_num_slave=n+1;
			lcd_print(1,1,mnp_for_slave,3);
			slave_command(mnp_for_slave);
			master_command(n);
		...
		if (s == 'B')
		{
			node_for_note(current_node_slave,note_string[n]);
			note_num_slave=n;
			lcd_print(1,1,mnp_for_slave,3);
			slave_command(mnp_for_slave);
			master_command(n+1);
		...
		if (s== 'C')
		{
			node_for_note(current_node_slave,note_string[n+2]);
			note_num_slave=n+2;
			lcd_print(1,1,mnp_for_slave,3);
			slave_command(mnp_for_slave);
			master_command(n);
			master_command(n+1);
			while(note_flag[n+2]==0)
			{
				delay(1);
			}
			n+=3;			
		...
		{
			node_for_note(current_node_slave,note_string[n+1]);
			slave_command(mnp_for_slave+40);
			node_for_note(current_node_slave,note_string[n]);
			slave_command(mnp_for_slave);
			master_command(n+2);
			while(note_flag[n+2]==0)
...
void init_pins()
{
	...
	//Servo Motor
	DDRB = DDRB | 0x20;			//making PB4 pin output
	PORTB = PORTB | 0x20;		//setting PB4 pin to logic 1
